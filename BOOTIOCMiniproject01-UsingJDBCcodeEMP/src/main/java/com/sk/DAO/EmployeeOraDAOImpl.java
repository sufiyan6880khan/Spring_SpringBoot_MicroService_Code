...
{
     // SELECT QUERY
	private static final  String  GET_EMP_QUERY  =  "SELECT EMPNO, ENAME ,  JOB, MGR, SAL, DEPTNO FROM EMP WHERE JOB IN (?,?,?) ";
		
	// INSERT QUERY
	private static final String INSERT_EMPLOYEE = "INSERT INTO EMP (EMPNO ,  ENAME,	 JOB,  MGR,  SAL , COMM, DEPTNO)  VALUES (EMP_SEQ1.NEXTVAL,?,?,?,?,?,?)";
	
	// DELETE QUERY 
	private static final String DELETE_EMPLOYEE = "DELETE FROM EMP WHERE EMPNO = ?";	
	
	// UPDATE OPERATION
	private static final String UPDATE_EMPLOYEE = " UPDATE  EMP SET  SAL = ?,  COMM = ? WHERE EMPNO = ? ";
...
// Update Employee by empno...
	...
	public int updateEmployee(Employee emp) throws Exception {
				   ...
				   PreparedStatement ps = con.prepareStatement(UPDATE_EMPLOYEE);) 
		   {
			          ...
			          // SET VALUE TO Parameter
			              ps.setInt(1, emp.getBsal());
			              ps.setInt(2, emp.getComm());
